...
- **PentaFabric**: canonical orchestration fabric. Founder alias `prenrafrabic` is preserved as an alias without creating a second runtime identity.
...
- **PentaSecure**: security-finding packaging and security release classification.
- **PentaVault**: trade-secret and proprietary implementation custody.
...
0. Founder and Reserved Authority: D3 remains human-reserved.
...
1. inspect actual recent failure evidence;
2. create reproducible, bounded fix requests;
3. hand valid requests to PentaFactory;
4. require security, test, package, deploy, assurance, and rollback evidence;
5. package ordinary bug fixes as patch candidates;
6. package every active security finding separately;
7. classify critical security fixes as hotfix candidates with a security appendix policy;
8. preserve major-version decisions as governed candidates rather than inferring a major release automatically.

## Proprietary custody

Penta algorithms, runtime code, private security logic, calibration, and implementation details are CrownThrive proprietary assets. Public registries disclose stable IDs, purpose, interfaces, DIDs, versions, and public-reference digests. Exact implementation bodies remain in Supabase Vault or other governed restricted custody.

Current custody includes the Generation 61 maintenance, release, job dispatch, PentaSpecter, PentaSecure, PentaWave, PentaEdge, PentaMesh, PentaVault, generational handoff, status-service, Control API, and MCP implementation assets.

Public repository visibility does not grant a license to protected Penta implementations.

## API and MCP

Two authenticated controlled-test surfaces are provider-deployed:

- `ct.system.penta-control-api.v1`: Penta Control API, deployment version 2.
- `ct.system.penta-mcp.v1`: PentaFabric MCP, deployment version 2, targeting protocol `2026-07-28`.

Both require JWT authentication. Provider deployment evidence exists; authenticated independent end-to-end invocation remains a separate verification state. Neither interface grants provider-write, money-movement, rights-grant, D3, public-activation, or self-approval authority.

## Distribution topology

Parent source authority: `crownthrive1/chlom-protocol`.
...
PentaFabric is a CrownThrive commercial software and institutional automation product family in controlled-test development. Product packaging, licensing candidates, SDK/API/MCP contracts, training, managed services, and conformance artifacts may be prepared. Prices, checkout, customer entitlements, rights grants, external certification, or commercial activation require separate governed authority.
